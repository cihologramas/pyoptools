    ...
    for i in range(q+1):
        for j in range(B+1):
            for k in range(B-j+1):
                c = pow(-1, i+j)*binomial(am, 2*i+p)*binomial(B-j, k)*int(factorial(n-j)
                                                                          )/(int(factorial(j))*int(factorial(mm-j))*int(factorial(n-mm-j)))
                x_pow = 2*(i+k)+p
                y_pow = n-2*(i+j+k)-p
...
class ZernikeXY(object):
    ...
    **ARGUMENTS**
    ...
    cohef = property(__get_cohef__, __set_cohef__, None,
