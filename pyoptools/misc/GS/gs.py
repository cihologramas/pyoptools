...
def gs_mod(idata, itera=10, osize=256):
    ...
    **ARGUMENTS:**
    ...
    fdata = fftshift(fft2(ifftshift(idata+mask)))
...
def gs_gpu(idata, itera=100):
    ...
    **ARGUMENTS:**
    ...
    plan = Plan(idata.shape, queue=queue, dtype=complex128)
    ...
    idata_gpu = cl_array.to_device(
        queue, ifftshift(idata).astype("complex128"))
...
def gs_mod_gpu(idata, itera=10, osize=256):
    ...
    idata_gpu = cl_array.to_device(
        queue, ifftshift(idata+mask).astype("complex128"))
    fdata_gpu = cl_array.empty_like(idata_gpu)
    rdata_gpu = cl_array.empty_like(idata_gpu)
    error_gpu = cl_array.to_device(
        ctx, queue, zeros(idata_gpu.shape).astype("double"))
    plan.execute(idata_gpu.data, fdata_gpu.data)
    ...
    for i in range(itera):
        ...
        norm1(queue, rdata_gpu.shape, None, rdata_gpu.data,
              idata_gpu.data, error_gpu.data, int32(cut))
